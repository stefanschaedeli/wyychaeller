...
- Storage locations track exactly where bottles are: a simple named location or a grid with rows and slots. Right after the label photo an overlay asks where the bottles go: pick a location from the list, tap the slot for each bottle and confirm with the green check; then the next label can be photographed. «Mehr → Lagerorte» shows every location with its wines and how full it is.
